...
- **`spawn_role`** — spawn a role-scoped subagent (researcher, coder, reviewer, planner, debugger) with persona injection, tool whitelist, step limit, model override, and depth-limited recursion. Foreground mode waits for completion; background mode returns an `agentId` immediately and can be joined later. Optional `resultConstraints` dynamically restrict typed reviewer criterion IDs, evidence IDs, and artifact URIs so invalid references are repaired inside the child session before they reach a parent completion gate.
- **`dag_execute`** — execute an admitted DAG with adaptive ready-node scheduling (or legacy wave barriers), bounded concurrency, write-scope leases, result aggregation, and upstream-data injection. Bounded result-driven dispatch can add validated scheduler-visible children from a dispatcher's structured report. New nodes can opt into the semantic `expected_output` + `consumers` contract; leaf nodes declare `$result`. The `role` field remains optional for legacy/default subagents. An explicit `retryPolicy` enables infrastructure-only retries and returns a typed `attempts` history; omitting it preserves the historical one-attempt behavior.
- **`dag_resume`** — resume a DAG from a V1 wave or V2 explicit-node checkpoint without replaying terminal nodes or already-expanded dispatchers. It accepts the same optional `retryPolicy` as first execution and returns the resumed attempt history.
- **`workflow_execute`** — execute the shared V1 workflow contract for direct, sequential, parallel, conditional, bounded loop, map/reduce, handoff, and DAG workflows. Acyclic kinds compile into `dag_execute`; loop iterations use a bounded runtime with the same role/resource adapter and typed retry policy.
